...
#########################
DELIMITER $$
CREATE PROCEDURE add_emprestimo_periodico(IN _id_periodico INT, IN _id_pessoa INT, IN _data_devolucao DATE, OUT _disponivel BOOLEAN)
BEGIN
...
DELIMITER $$
CREATE PROCEDURE add_periodico(IN _issn INT, IN _volume INT, IN _ano INT, IN _descricao VARCHAR(50), IN _titulo VARCHAR(20), IN _codigobarras VARCHAR(13), IN _estante INT, IN _exemplares INT, OUT _gravado BOOLEAN)
...
DELIMITER $$
CREATE PROCEDURE add_exemplares_periodico(IN _issn INT, IN qtde_add INT, OUT _atualizado BOOLEAN)
BEGIN
...
DELIMITER $$
CREATE PROCEDURE devolucao_periodico(IN _id_periodico INT, IN _id_pessoa INT, IN _id_emprestimo INT, OUT _devolucao_efetuada BOOLEAN, OUT _existe_emprestimo BOOLEAN)
BEGIN
...
#####################
DELIMITER $$
CREATE PROCEDURE add_emprestimo_livro(IN _id_livro INT, IN _id_pessoa INT, IN _data_devolucao DATE, OUT _disponivel BOOLEAN)
BEGIN
...
DELIMITER $$
CREATE PROCEDURE add_livro(IN _isbn INT, IN _id_editora INT, IN _volume INT, IN _ano INT, IN _edicao INT, IN _titulo VARCHAR(20), IN _codigobarras VARCHAR(13), IN _estante INT, IN _exemplares INT, OUT _gravado BOOLEAN)
...
DELIMITER $$
CREATE PROCEDURE add_exemplares_livro(IN _isbn INT, IN qtde_add INT, OUT _atualizado BOOLEAN)
BEGIN
DECLARE $exemplares, $disponiveis INT;
SET _atualizado = FALSE;
...
DELIMITER $$
CREATE PROCEDURE devolucao_livro(IN _id_livro INT, IN _id_pessoa INT, IN _id_emprestimo INT, OUT _devolucao_efetuada BOOLEAN, OUT _existe_emprestimo BOOLEAN)
BEGIN
...
DELIMITER $$
CREATE PROCEDURE add_material_especial(IN _id_material_especial INT, IN _id_tipo_material_especial INT, IN _titulo VARCHAR(20), IN _codigobarras VARCHAR(13), IN _estante INT, IN _exemplares INT, IN _descricao VARCHAR(250), OUT _gravado BOOLEAN)
#PROCEDURE RESPONSÁVEL POR ADICIONAR MATERIAIS ESPECIAIS AO BANCO DE DADOS
#UMA DAS FUNÇÕES É CERTIFICAR QUE O NÚMERO DE EXEMPLARES ADICIONADOS 
#SEJA IGUAL AO NÚMERO DE EXEMPLARES DISPONÍVEIS
BEGIN
SET _gravado = FALSE;
INSERT INTO materialespecial(id_material_especial, id_tipo_material_especial, titulo, codigobarras, estante, exemplares, disponiveis, descricao)
VALUES(_id_material_especial, _id_tipo_material_especial, _titulo, _codigobarras, _estante, _exemplares, _exemplares, _descricao);
SET _gravado = TRUE;
END$$
DELIMITER ;
